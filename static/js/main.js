document.addEventListener('DOMContentLoaded', () => {
        ...
        videoForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (channelName.includes('Sri Sathya Sai') || channelName.includes('Prasanthi') || channelName.includes('Billy') || channelName.includes('ariefngurahrai')){
    ...
    if (videoInfo) {
        videoInfo.addEventListener("click", function (event) {
            event.preventDefault();  // Prevent form submission
            console.log("Hi");
            quoTe.style.display = 'none';
            const video_info = document.getElementById("video-info")
            if (video_info) {
                if (video_info.style.display == 'none') {
            ...
            if (channel_info.style.display == 'block') {
    ...
    if (channelInfo) {
        channelInfo.addEventListener("click", function (event) {
            event.preventDefault();  // Prevent form submission
            console.log("Hi to channel");
            quoTe.style.display = 'none';
            const video_info = document.getElementById("video-info")
            if (video_info.style.display == 'block') {
                video_info.style.display = 'none';
            }
            const channel_info = document.getElementById("channel-info")
            if (channel_info) {
                if (channel_info.style.display == 'none') {
                    channel_info.style.display = 'block';
                } else {
                    channel_info.style.display = 'none';
                    quoTe.style.display = 'block';
                }
